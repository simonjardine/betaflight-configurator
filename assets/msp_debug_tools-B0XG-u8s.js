...
Happy debugging! 🚀
`);(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1")&&(console.log("🔄 Development environment detected - auto-starting basic monitoring"),x(async()=>{const{mspQueueMonitor:r}=await Promise.resolve().then(()=>q);return{mspQueueMonitor:r}},void 0).then(({mspQueueMonitor:r})=>{r.addListener(e=>{Object.values(e.alerts).filter(s=>s).length>0&&console.warn("🚨 MSP Alert detected - check dashboard for details")}),r.startMonitoring(2e3)}));
